...
public class BaseItemScript : MonoBehaviour
{
	...
	private void OnDestroy()
	{
		if (connectedItems != null)
		{
			...
			{
				Scenman.instance.RemoveItem(connectedItems[index]);
			}
		}
	}
	...
	public void UpdateConnectedItems()
	{
		...
		if (this.itemData.name == "Tower")
		{
			...
			if (towerArcher == null)
			{
				towerArcher = Scenman.instance.AddItem(1502, true, ownedItem);
		...
		if (this.itemData.name == "BuilderHut" && Scenman.instance.gameMode == Common.GameMode.NORMAL)
		{
			if (Scenman.instance.selectedItem == this)
			{
			...
			if (builder == null)
			{
				builder = Scenman.instance.AddItem(3823, true, ownedItem);
	...
	void Update()
	{
		...
		if (Scenman.instance.gameMode == Common.GameMode.NORMAL && this.itemData.configuration.productionRate > 0)
